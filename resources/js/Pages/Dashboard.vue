...
const props = withDefaults(defineProps<{ user: type.User; groups: type.Group[] }>(), {});
...
const tasksToday = ref<type.Group[]>(
    groups.value.map(group => ({
        ...group,
        tasks: group.tasks
            .filter(e => new Date(e.appointment || e.created_at).toLocaleDateString() == new Date().toLocaleDateString())
                    ...
                    <template v-if="e.tasks.length > 0">
                        ...
                        <div v-for="task of e.tasks">
                            <Modal :title="task.title">
                                <showTask dashboard :task="task" :group="e" :groupUser="e.users" :user="user" :task-id="task.id"></showTask>
                                <template #button>
                                    <div class="m-1">
                                        <Button class="btn btn-primary w-100">
                                            {{ task.title }} ({{ new Date(task.appointment || task.created_at).toLocaleDateString() }})
                                        </Button>
                                    </div>
                                </template>
                            </Modal>
